...
public class JobController{
	...
	public HashMap<String, List<Job>> create(@RequestBody  Job job){
		logger.debug("is customer null "+job.getCustomer().getId());
...
//		System.out.println(job.getId());
	...
	public  void jobCompleted(@RequestBody Job job) throws IOException {
		...
		logger.debug("is user passing "+job.getUser());
		...
		logger.debug("is job id null "+ jobId);
	...
	public HashMap<String,List> getSingleJob(@RequestBody Job job){
		logger.info("getting single job "+job.getId());
		...
		logger.info("how many doors "+jobProperties.get("doors").size());
